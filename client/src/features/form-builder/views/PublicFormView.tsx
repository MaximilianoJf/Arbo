        ...
        return (
            <div className="min-h-dvh flex flex-col px-4 py-8" style={{ background: pageBg, ...animVars }}>
                <div ref={gridRef} className={`relative w-full max-w-5xl mx-auto ${styles.pageImageSnap === "bottom" ? "flex-1" : ""}`} style={{ minHeight: containerMinH }}>
                    ...
                    {/* Footer: placed at its exact grid row.
                        containerMinH (via pageLayoutHeight) already ensures the container
                        is tall enough, so no contentBottom heuristic needed here. */}
                    {footerItem && (
                        <div style={{ position: "absolute", top: footerItem.y * rowH, left: 0, width: "100%" }}><Footer /></div>
                    )}
                </div>
                {!footerItem && <Footer />}
            </div>
